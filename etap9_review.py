...
DIAGNOSE_SYSTEM = """Ты — viktor, code reviewer. Проект ПРОВАЛИЛСЯ. Тебе дают цель,
причину провала и содержимое изменённых файлов. Твоя задача — поставить ДИАГНОЗ:
объяснить простым языком, ЧТО именно пошло не так в коде и ЧТО нужно поправить.
Это не блокировка (проект уже провален) — это объяснение для человека у «стекла».

Ответь СТРОГО одним JSON-объектом без пояснений и markdown:
{"diagnosis":"<краткое объяснение причины провала на русском, 1-3 предложения>","fixes":["<конкретный шаг исправления>"]}"""


def diagnose(plan_id: str, goal: str, reason: str, root: str, llm) -> dict:
    """viktor-диагност при провале проекта: читает изменённый код и объясняет
    в ленте, что сломалось. Не блокирует (проект уже провален), только поясняет.
    Безопасна: при любой ошибке/отсутствии данных просто молчит."""
    if llm is None or not root:
        return {}
    files = _changed_files(root)
    if not files:
        return {}
    try:
        events.publish("diagnose_started", agent="viktor", plan_id=plan_id)
        user = (f"ЦЕЛЬ ПРОЕКТА: {goal}\n\nПРИЧИНА ПРОВАЛА: {reason}\n\n"
                f"ИЗМЕНЁННЫЕ ФАЙЛЫ:\n{_collect(root, files)}")
        result = _parse(llm.complete(DIAGNOSE_SYSTEM, user))
        diagnosis = str(result.get("diagnosis", "")).strip()
        fixes = result.get("fixes", []) if isinstance(result.get("fixes"), list) else []
        if diagnosis:
            events.publish("diagnosis", agent="viktor", plan_id=plan_id,
                           diagnosis=diagnosis[:500],
                           fixes=[str(f)[:200] for f in fixes[:5]])
        return {"diagnosis": diagnosis, "fixes": fixes}
    except Exception as e:  # диагност не должен ронять обработку провала
        events.publish("diagnose_failed", agent="viktor", plan_id=plan_id,
                       detail=str(e)[:200])
        return {}
